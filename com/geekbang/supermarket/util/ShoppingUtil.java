...
public class ShoppingUtil {
    ...
    public static SuperMarket create2() {
        Merchandise[] alias = new Merchandise[100];
        for (Category category : Category.values()) {
            for (int i = 0; i < 10; i ++){
                ...
                alias[category.ordinal() * merchandisePerCategory + i] = new SimpleMerchandise(
                        category.name() + i, soldPrice, purchasePrice, 200, category);
            }
        }
        SimpleSuperMarket superMarket = new SimpleSuperMarket(alias);
        output("请输入超市的名字：");
        String s = input().next();
        if (s.trim().length() > 0) {
